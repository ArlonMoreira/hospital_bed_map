...
import { register as registerBed, resetAlertBed } from '../../slices/bedSlice';
...
import { IStatus, IBedParams, IType } from '../../interfaces/Bed';
//Component
...
const Hospital = (props: Props) => {
    ...
    useEffect(()=>{
        if(statusData && statusData.length > 0){
            const data = statusData as IStatus[];
            setStatusBed(data[0].id);
        }
    ...
    useEffect(()=>{
        if(typeData && typeData.length > 0){
            const data = typeData as IType[];
            setTypeBed(data[0].id);
        }

    }, [typeData]);

    const handleRegisterBed = async(e:FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        ...
        await dispatch(refreshToken()); //Update authentication tokens to fetch data from the backend
        await dispatch(registerBed(data))
            ...
            <div className='modal fade pb-5' id='add-bed-modal' data-bs-backdrop="static" data-bs-keyboard="false" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                <div className='modal-dialog modal-dialog-centered modal-lg'>
                    <div className='modal-content border-0'>
                        ...
                        <form className='register_form' onSubmit={handleRegisterBed}>
                            <div className='modal-body p-4 row'>
                                ...
                                <label className='col-4 d-flex flex-column'>
                                    <span>Ativo:</span>
